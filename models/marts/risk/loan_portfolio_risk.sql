...
loan_performance as (
    select
        ...
        -- repayments include interest, so paid can exceed principal: floor at 0
        ...
        case
            ...
            else datediff('day', l.maturity_date, current_date)
        ...
        case
            ...
            when datediff('day', l.maturity_date, current_date) <= 30 then 'dpd_1_30'
            when datediff('day', l.maturity_date, current_date) <= 60 then 'dpd_31_60'
            when datediff('day', l.maturity_date, current_date) <= 90 then 'dpd_61_90'
            else 'npl'  -- non-performing: more than 90 days past due
        end                                                  as loan_status
    ...
    select
        *,
        ...
        case when loan_status = 'npl' then outstanding_balance_usd else 0 end
            as npl_exposure_usd,
        -- IFRS9-style provisioning ladder by delinquency stage
        outstanding_balance_usd * case loan_status
            when 'active'    then 0.05
            when 'dpd_1_30'  then 0.15
            when 'dpd_31_60' then 0.30
            when 'dpd_61_90' then 0.50
            when 'npl'       then 1.00
            else 0.0
        end                             as provision_required_usd
